...
or configure `AGENT_INTAKE_ENGINE=deepseek` in the process, Windows User, or
Windows Machine environment when external API refinement is explicitly allowed.
Review digest parsing follows the same rule for
