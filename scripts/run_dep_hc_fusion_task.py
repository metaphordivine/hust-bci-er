"""Run a validation-selected score fusion for independent DEP/HC feature branches."""
...
from hust_bci_er.tasks.dep_hc.fusion import evaluate_dep_hc_feature_fusion  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run DEP/HC validation-selected feature-score fusion.")
    ...
    parser.add_argument("--feature-set", action="append", choices=sorted(DEP_HC_FEATURE_SETS), required=True)
    ...
    if len(args.feature_set) != 2:
        parser.error("--feature-set must be supplied exactly twice for two-way fusion")
    if any(feature.startswith("traditional") for feature in args.feature_set) and args.channel_montage != "hust_30_a2":
        parser.error("traditional feature sets require --channel-montage hust_30_a2")
