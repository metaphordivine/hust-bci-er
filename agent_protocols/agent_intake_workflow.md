...
DeepSeek-first intake is the default when `DEEPSEEK_API_KEY` is available. If
the key or API is unavailable, `scripts/agent_intake.py` falls back to the
deterministic classifier. Use `--deterministic` to force local-only intake, or
